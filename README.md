...
当前 GitHub hosted mirror 已可直接引用。  
如果你是在同一工作区里并排维护三个仓，继续使用 sibling-path 也没有问题；但对外消费时，推荐直接使用下面这组 hosted 依赖。

```toml
[dependencies]
jinguissl_core = { git = "git@github:Celading/jinguisslCore", branch = "main" }
jinguissl_contract = { git = "git@github:Celading/JinguiSSL", branch = "main" }
jinguissl_bridge = { git = "git@github:Celading/jinguisslBridge", branch = "main" }
```
